...
//             exmo.CW = idex.CW;
//             exmo.rdl = idex.rdl;
//             string sign_extend_b=SignedExtend(idex.imm);
...
public:
    ...
    void execute()
    {
        string ALUSelect = ALUControl(idex.CW.ALUOp, IM->instruction(25, 31), idex.func3);
        ALU alu(ALUSelect, idex.rs1, idex.rs2);
        exmo.ALUOut = alu.ALUresult;
        int ALUZeroFlag = alu.ALUZeroFlag;
        exmo.CW = idex.CW;
        ...
        {
            IM->pc += 4; // Increment PC by 4 for next instruction
        }
    ...
    void MemOp()
    {
        if (exmo.CW.MemWrite)
        {
            DM[exmo.ALUOut] = idex.rs2; // Write to data memory
        }
